...
class iCagendaModelregistration extends JModelAdmin
{
	...
	public function save($data)
	{
		...
		$extraTickets       = ($data['people'] - $data['data_people']);
		$ticketsBookable    = ($data['data_people'] + $ticketsAvailable);

		if ($data['people'] > $ticketsBookable)
		{
			$msg = '<strong>' . JText::_('COM_ICAGENDA_REGISTRATION_ERROR_NOT_ENOUGH_TICKETS_AVAILABLE') . '</strong><br />';

			if ($data['data_people'])
			{
				$msg.= JText::sprintf('COM_ICAGENDA_REGISTRATION_ERROR_NOT_ENOUGH_TICKETS_ALREADY_BOOKED', $data['data_people']);
				$msg.= JText::sprintf('COM_ICAGENDA_REGISTRATION_ERROR_NOT_ENOUGH_TICKETS_EXTRA_PEOPLE', $extraTickets, $ticketsAvailable)
					. '<br />';
			}

			$msg.= '<strong>' . JText::sprintf('COM_ICAGENDA_REGISTRATION_ERROR_MAXIMUM_TICKETS_BOOKABLE', $ticketsBookable)
				. '</strong>'
				. '<br />';
			$msg.= JText::_('COM_ICAGENDA_REGISTRATION_ERROR_NOT_ENOUGH_TICKETS_CHANGE_NUMBER_OF_PEOPLE');
...
//			$msg.= '<br />$ticketsAvailable: ' . $ticketsAvailable;
		...
		{
			$msg = '<strong>' . JText::_('COM_ICAGENDA_REGISTRATION_NO_DATE_SELECTED') . '</strong>';
