			...
			<fieldset id="filter-bar">
				...
				<div class="filter-select fltrt">
					<select name="filter_group" class="inputbox" onchange="this.form.submit()">
						<option value=""><?php echo JText::_('COM_ICAGENDA_CUSTOMFIELDS_FILTER_OPTION_SELECT_GROUP');?></option>
						<?php echo JHtml::_('select.options', $this->get('Groups'), "value", "text", $this->state->get('filter.group'), true);?>
					</select>
				</div>
...
// Yellow		$rowStyle = $overrideCore ? ' style="background: #fcf8e3; color: #8a6d3b;"' : '';
// Red			$rowStyle = $overrideCore ? ' style="background: #f2dede; color: #a94442;"' : '';
// Green		$rowStyle = $overrideCore ? ' style="background: #dff0d8; color: #3c763d;"' :
//				($item->parent_form == 2 ? ' style="background: #d9edf7;"' : '');
//				$rowClass = $overrideCore ? ' info' : '';
				?>
...
//												? str_replace($type_prefix . '_', '' , $item->type)
				...
				<?php if(version_compare(JVERSION, '3.0', 'lt')) : ?>
					...
					<?php if (isset($this->items[0]->ordering)) { ?>
						<td class="order"<?php echo $rowStyle;?>>
							<?php if ($canChange) : ?>
								<?php if ($saveOrder) :?>
									<?php if ($listDirn == 'asc') : ?>
